    ...
    def __init__(self, config, img_c, img_sz):
        ...
        for i, (name, param) in enumerate(self.config):
            if name == 'conv2d':
                ...
                w = nn.Parameter(torch.ones(param[1],param[0],3,3)) #con3x3
                torch.nn.init.kaiming_normal_(w)
                self.vars.append(w)
                self.vars.append(nn.Parameter(torch.zeros(param[1])))
                
                
                w = nn.Parameter(torch.ones(param[1])) #batchnorm
                self.vars.append(w)
                # [ch_out]
                self.vars.append(nn.Parameter(torch.zeros(param[1])))
                ...
                w = nn.Parameter(torch.ones(param[1],param[1],3,3)) #con3x3
                torch.nn.init.kaiming_normal_(w)
                self.vars.append(w)
                self.vars.append(nn.Parameter(torch.zeros(param[1])))                
